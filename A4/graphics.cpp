...
void window::move_selected_object(float changeX, float changeY) {
    if(selected_obj < objects.size()){
        float deltaX = changeX * window::moveBy;
        float deltaY = changeY * window::moveBy;
        float w_width = window::width;
        float w_height = window::height;
        if(window::selected_obj >= window::objects.size()) return;
        window::objects[window::selected_obj].move(deltaX, deltaY,
                                                w_width, w_height);
    }
}
...
void window::display() {
    ...
    if(window::selected_obj < window::objects.size()){
        glLineWidth(window::borderThick);
        glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
        window::objects[selected_obj].draw(window::borderColor);
    }
    mus.draw();
    glutSwapBuffers();
}
    ...
    switch (key) {
        ...
        case 'P': case 'p': case BS:
            ...
            if(window::selected_obj == 0) {
                newPos = window::objects.size() - 1;
            }
            else {
                newPos = window::selected_obj - 1;
            }
            window::select_object(newPos);
            break;
        case '0'...'9':
            //select_object (key - '0');
